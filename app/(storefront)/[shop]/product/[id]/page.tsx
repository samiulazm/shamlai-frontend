...
export default function ProductPage({ params }: { params: { id: string } }){
    ...
    return (
      <div className="container-responsive py-10">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">{error || 'This product does not exist or is no longer available.'}</p>
  ...
  const currentPrice = selectedVariant && variants.length > 0
    ...
    <div className="container-responsive py-10 grid gap-8 md:grid-cols-2">
      ...
      <div>
        ...
        <div className="mt-4 flex items-baseline gap-3">
          <span className="text-3xl font-semibold">{currencySymbol}{currentPrice.toFixed(2)}</span>
        ...
        <div className="mt-6 grid gap-3">
          ...
          <button 
            ...
            disabled={addingToCart || (product.inventory_quantity !== null && product.inventory_quantity === 0)}
            className="btn btn-primary w-full"
          >
            {addingToCart ? 'Adding...' : product.inventory_quantity === 0 ? 'Out of Stock' : 'Add to Cart'}
          </button>
        </div>
      </div>
    </div>
