...
export async function POST(request: NextRequest) {
  try {
    ...
    if (verifyError || !event) {
      logger.error('Webhook signature verification failed', verifyError instanceof Error ? verifyError : new Error(String(verifyError)));
      return NextResponse.json(
    ...
    if (!success) {
      logger.error('Webhook event handler failed', handlerError instanceof Error ? handlerError : new Error(String(handlerError)));
      return NextResponse.json(
