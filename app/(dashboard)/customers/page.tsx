...
export default function Customers() {
  ...
  const fetchCustomers = async () => {
    try {
      ...
      if (ordersError) {
        logger.warn(
          'Error fetching orders for customer stats',
          ordersError instanceof Error ? ordersError : new Error(String(ordersError))
        );
      }

      // Create a map for O(1) order lookups (performance optimization)
      const ordersByCustomer = new Map<string, typeof ordersData>();
      ...
      // Calculate stats for each customer (optimized)
      const customersWithStats = (customersData || []).map((customer) => {
        ...
        // Find last order date without sorting entire array
        const lastOrderDate =
        ...
        <div className="card">
          <div className="card-pad">
            <div className="text-center py-8">
              ...
              <button onClick={fetchCustomers} className="btn btn-outline">
                Try Again
              </button>
            </div>
          </div>
              ...
              <tbody>
                {customers.map((customer) => (
                  <tr key={customer.id}>
                    ...
                    <td>
                      <span className="badge bg-blue-100 text-blue-800">{customer.orderCount}</span>
                    </td>
                    ...
                    <td className="text-sm text-gray-600">{formatDate(customer.created_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
