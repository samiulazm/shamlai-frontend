...
const products = await cacheProductsList(shopId, { category: 'electronics', isActive: true }, () =>
  fetchProducts(shopId, filters)
);

// Cache analytics data
const stats = await cacheAnalytics(shopId, 'revenue', { period: '30d' }, () =>
...
```typescript
import { invalidateProductCache, invalidateOrderCache, invalidateShopCache } from '@/lib/cache';
...
const allOrders = await Promise.all([getActiveOrders(customerId), getArchivedOrders(customerId)]);
